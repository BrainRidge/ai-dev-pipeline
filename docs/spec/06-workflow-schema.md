...
| `toolCheck` | Probes the machine for the tools the workflow needs | No required tool is missing, and the developer continues ([Section 17](17-tool-check.md)) |

| `taskType` | `stepType` | What it does |
|---|---|---|
| `toolCheck` | `toolCheck` | Reports which of the team's tools are installed |
...
| File | Id | Label | Steps |
|---|---|---|---|
| `researchTaskWorkflow_1_0.json` | `researchTaskWorkflow` | Research Task | toolCheck → requirement → gitClone → aiHandoff → reviewAnalysis |
| `newFeatureWorkflow_1_0.json` | `newFeatureWorkflow` | New Feature | toolCheck → requirement → gitClone → aiHandoff → reviewAnalysis → CodeImplementation → CodeReview |
| `bugFixWorkflow_1_0.json` | `bugFixWorkflow` | Bug Fix | toolCheck → requirement → gitClone → diagnosis → reviewDiagnosis → CodeFix → CodeReview |

Every workflow opens on `toolCheck`. Nothing in the schema requires that — a
...
`toolCheck` step looks for. Unlike the two above it is optional and falls back
