...
import { WebviewBridge, type Transport } from '../../src/bridge/WebviewBridge'
...
describe('a retained webview that comes back', () => {
  function seam() {
    ...
    let onMessage: Parameters<Transport['onMessage']>[0] | undefined
    // A Transport rather than a fake Webview: the bridge is host-independent, so
    // its test needs no IDE type and no `as never`. See spec Section 19.
    ...
    const bridge = new WebviewBridge<{ id: string }>(transport)
    return {
      bridge,
      posted,
      ready: () => onMessage?.({ type: 'ready' } as Parameters<NonNullable<typeof onMessage>>[0]),
    }
  }
