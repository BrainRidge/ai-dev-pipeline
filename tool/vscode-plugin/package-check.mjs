...
export function wouldShip(cwd) {
  // --no-dependencies because the extension ships one esbuild bundle and never
  // its node_modules, which .vscodeignore has always excluded. Without the flag
  // vsce resolves production dependencies through the npm workspace, decides the
  // repository root is the package root, and lists 5,000 files from outside this
  // folder — including .git. See spec Section 19.
  return execFileSync('npx', ['vsce', 'ls', '--no-dependencies'], {
    cwd,
    encoding: 'utf8',
