...
var AuditLog = class {
  ...
  file;
  constructor(taskDir) {
    this.engineDir = (0, import_node_path.join)(taskDir, ".engine");
    this.file = (0, import_node_path.join)(this.engineDir, "audit.jsonl");
  }
  async append(entry) {
    await (0, import_promises.mkdir)(this.engineDir, { recursive: true });
    const stamped = { at: (/* @__PURE__ */ new Date()).toISOString(), ...entry };
    await (0, import_promises.appendFile)(this.file, `${JSON.stringify(stamped)}
`, "utf8");
  }
  async entries() {
    const raw = await (0, import_promises.readFile)(this.file, "utf8");
    return raw.trim().split("\n").filter(Boolean).map((l) => JSON.parse(l));
  }
};

// ../core/src/audit/summary.ts
function summariseHandoffs(perTask) {
  const summary = {
...
// ../core/src/bridge/WebviewBridge.ts
...
// ../core/src/content/ContentRoot.ts
var import_promises2 = require("node:fs/promises");
var import_node_path2 = require("node:path");

// ../core/src/session/SetupSelection.ts
...
function derivedFrom(root) {
  return {
    microserviceConfig: (0, import_node_path2.join)(root, "config", "microservices.json"),
    platformConfig: (0, import_node_path2.join)(root, "config", "platforms.json"),
    customPrompts: (0, import_node_path2.join)(root, "prompts"),
    toolsConfig: (0, import_node_path2.join)(root, "config", "tools.json")
  };
}
...
var nodeProbe = {
  async list(dir) {
    try {
      return await (0, import_promises2.readdir)(dir);
    } catch {
      return void 0;
    }
  }
...
function templateResolver(opts, probe) {
  return async (relativePath) => {
    const expected = (0, import_node_path2.basename)(relativePath);
    const bundled = {
      path: (0, import_node_path2.join)(opts.bundledPromptsDir, relativePath),
      source: "bundled"
    };
    if (!opts.promptsDir) return bundled;
    const dir = (0, import_node_path2.join)(opts.promptsDir, (0, import_node_path2.dirname)(relativePath));
    const names = await probe.list(dir);
    if (!names) return bundled;
    if (names.includes(expected)) return { path: (0, import_node_path2.join)(dir, expected), source: "external" };
...
async function externalWorkflowsPresent(root, probe) {
  return await probe.list((0, import_node_path2.join)(root, "workflows")) !== void 0;
}
...
// ../core/src/engine/StepDescriptor.ts
...
function summarise(step, record, fields) {
  ...
  switch (step.stepType) {
    ...
    default: {
      ...
      const parts = (fields ?? []).map((f) => answers[f.id]).filter((v) => v !== void 0 && v !== null && v !== "").map((v) => Array.isArray(v) ? v.join(", ") : String(v));
...
function detailAnswers(step, record, fields) {
  ...
  const pairs = (fields ?? []).map((f) => ({
    label: f.label,
    value: Array.isArray(answers[f.id]) ? answers[f.id].join(", ") : String(answers[f.id] ?? "")
  })).filter((p) => p.value !== "");
...
function artifactName(path) {
  return typeof path === "string" && path.length > 0 ? (0, import_node_path3.basename)(path) : void 0;
}

// ../core/src/engine/ToolCatalog.ts
...
var fieldSchema = external_exports.object({
  ...
  options: external_exports.array(external_exports.object({ value: external_exports.string(), label: external_exports.string() })).optional()
});
...
var microserviceSchema = external_exports.object({
  ...
  gitLocation: external_exports.string().min(1),
...
var platformsFileSchema = external_exports.object({
  ...
  platforms: external_exports.array(platformSchema)
});
function repoNameOf(gitLocation) {
  const trimmed = (gitLocation.split(/[?#]/)[0] ?? "").replace(/\/+$/, "");
  const path = trimmed.includes("://") ? trimmed.slice(trimmed.indexOf("://") + 3).replace(/^[^/]*/, "") : trimmed.slice(trimmed.indexOf(":") + 1);
...
// ../core/src/engine/WorkflowCatalog.ts
...
var WorkflowCatalog = class _WorkflowCatalog {
  ...
  static async load(workflowsDir2, config2) {
    ...
    for (const filename of await (0, import_promises4.readdir)(workflowsDir2)) {
      if (!filename.endsWith(".json")) continue;
      const parsed = parseWorkflowFilename(filename);
      if (!parsed) {
        throw new Error(
          `workflow filename "${filename}" is not versioned. Expected <name>_<major>_<minor>.json, e.g. researchTaskWorkflow_1_0.json`
        );
      }
      const file = workflowFileSchema.parse(
        JSON.parse(await (0, import_promises4.readFile)((0, import_node_path4.join)(workflowsDir2, filename), "utf8"))
      );
    ...
    return new _WorkflowCatalog(workflows, platforms, services);
  }
  get(id) {
  ...
  searchMicroservices(query) {
    ...
    return this.services.filter(
      (s) => s.microserviceName.toLowerCase().includes(q) || s.shortCode.toLowerCase().includes(q) || s.category.toLowerCase().includes(q)
    );
  }
...
function validateMicroservices(services) {
  ...
  const seenRepos = /* @__PURE__ */ new Map();
  for (const service of services) {
    const clashingCode = seenCodes.get(service.shortCode);
    if (clashingCode) {
      throw new Error(
        `microservices: "${service.microserviceName}" and "${clashingCode}" share the shortCode "${service.shortCode}"`
      );
    }
    ...
    if (!repo) {
      throw new Error(
        `microservices: "${service.microserviceName}" has a gitLocation with no repository name: "${service.gitLocation}"`
      );
    }
    const clashingRepo = seenRepos.get(repo);
    if (clashingRepo) {
      throw new Error(
        `microservices: "${service.microserviceName}" and "${clashingRepo}" both clone into "${repo}", so they would overwrite each other`
      );
    }
...
// ../core/src/engine/WorkflowEngine.ts
var WorkflowEngine = class {
  ...
  async submit(stepId, actionId, values) {
    ...
    await this.store.write(state);
    return { ok: true, done: step.nextStep === void 0 };
  }
  reopen(state, stepId) {
  ...
  previousId(fromId) {
    ...
    return index > 0 ? this.workflow.order[index - 1] : void 0;
  }
};

// ../core/src/engine/placeholders.ts
...
// ../core/src/prompt/PromptComposer.ts
...
var PromptComposer = class {
  ...
  async resolved(step, ctx) {
    ...
    return this.resolve(named ?? (0, import_node_path5.join)(ctx.workflowId, `${step.id}.md`));
  }
  /**
  ...
  async compose(step, ctx, repos) {
    ...
    const part5 = main.outputFile ? [
      ...
      "## Required output",
      "",
      `Write your result to \`${(0, import_node_path5.join)(ctx.taskDir, main.outputFile)}\`.`,
  ...
  async readParts(names, declaredIn, key) {
    ...
    for (const name of names) {
      ...
      let raw;
      try {
        raw = await (0, import_promises5.readFile)(resolved.path, "utf8");
      } catch {
        throw new Error(
  ...
  async resolveReferences(main, ctx) {
    ...
    for (const entry of main.reference) {
      ...
      const path = (0, import_node_path5.isAbsolute)(resolvedText) ? resolvedText : (await this.resolve(promptsRelative(resolvedText, main.path, "reference"))).path;
...
function promptsRelative(name, declaredIn, key) {
  ...
  if (trimmed === "" || windowsAbsolute || (0, import_node_path5.isAbsolute)(trimmed) || trimmed.split(/[\\/]/).includes("..")) {
    throw new Error(
...
// ../core/src/providers/ManualProvider.ts
...
// ../core/src/session/openFolders.ts
var import_node_path6 = require("node:path");
function isInside(path, folder) {
  const root = folder.endsWith(import_node_path6.sep) ? folder.slice(0, -import_node_path6.sep.length) : folder;
  return path === root || path.startsWith(root + import_node_path6.sep);
}
...
function resolveTasksRoot(configured) {
  return configured && configured.length > 0 ? configured : (0, import_node_path7.join)((0, import_node_os.homedir)(), "ai-dev-workflow", "tasks");
}
...
async function listUnfinishedTasks(tasksRoot2) {
  const entries = await (0, import_promises6.readdir)(tasksRoot2, { withFileTypes: true }).catch(() => []);
  const found = [];
  for (const entry of entries) {
    if (!entry.isDirectory() || entry.name.startsWith(".")) continue;
    const file = (0, import_node_path8.join)(tasksRoot2, entry.name, ".engine", "_state.json");
...
async function summarise2(taskId, file) {
  try {
    const state = JSON.parse(await (0, import_promises6.readFile)(file, "utf8"));
    ...
    return {
      ...
      updatedAt: (await (0, import_promises6.stat)(file)).mtimeMs
    };
  } catch {
    return void 0;
  }
...
function editedPrompt(values) {
  ...
  if (typeof edited !== "object" || edited === null) return void 0;
  const text = edited[PROMPT_BLOCK_ID];
...
// ../core/src/tasks/CopilotEditingHandoff.ts
var CopilotEditingHandoff = class {
  ...
  async describe(step, ctx, values) {
    ...
    return {
      text: failure ? `The prompt could not be composed: ${failure}` : `${this.instruction} Mark this step done once Copilot has finished and you have looked at what it changed.`,
      commands: block ? [block] : void 0,
      actions: [
        { id: "send", label: "Send to Copilot" },
  ...
  async copyPrompt(step, ctx, override) {
    const text = override ?? (await this.composer.compose(step, ctx, reposBefore(ctx, step.id))).prompt;
...
var GitClone = class {
  ...
  async deliver(blockId, mode, ctx) {
    const blocks = this.plan(ctx);
    ...
    const label = blockId === "all" ? `all ${blocks.length} repositories` : chosen[0]?.label ?? blockId;
  ...
  async execute(_step, ctx, _values) {
    return {
  ...
  selected(ctx) {
    ...
    return codes.map((code) => ctx.microservices.find((s) => s.shortCode === code)).filter((s) => s !== void 0);
  }
};
function folderFor(service) {
  return repoNameOf(service.gitLocation) ?? service.shortCode;
}
function baseBranchOf(ctx) {
...
var InvokeCopilot = class {
  ...
  async describe(step, ctx, values) {
    ...
    return {
      text: failure ? `The prompt could not be composed: ${failure}` : "Read the prompt below \u2014 edit it if you want to \u2014 send it to Copilot, then mark this step done once the output file has been written.",
      commands: block ? [block] : void 0,
      actions: [
        { id: "send", label: "Send to Copilot" },
        { id: "done", label: "Done", primary: true }
...
// ../core/src/tasks/InvokeCopilotCodeReview.ts
var InvokeCopilotCodeReview = class extends CopilotEditingHandoff {
...
// ../core/src/tasks/InvokeCopilotCoding.ts
var InvokeCopilotCoding = class extends CopilotEditingHandoff {
...
// ../core/src/update/UpdateCheck.ts
...
async function checkForUpdate(deps) {
  if (!deps.manifestUrl) return void 0;
  try {
    const manifest = await deps.fetchJson(deps.manifestUrl);
    return isNewer(deps.currentVersion, manifest.version) ? manifest.version : void 0;
  } catch {
    return void 0;
  }
}
...
function vscodeTransport(webview) {
  return {
    post(message) {
      void webview.postMessage(message);
    },
    onMessage(handler) {
      webview.onDidReceiveMessage((msg) => handler(msg));
    }
  };
}
function panelHtml(webview, scriptUri, styleUri, nonce) {
...
var VsCodeHost = class {
  handoff = new ChatHandoff();
  editorVersion = vscode2.version;
  skills = VSCODE_SKILLS;
  sink = {
    async copy(text) {
    ...
    async toTerminal(text) {
      const terminal = vscode2.window.terminals.find((t) => t.name === TERMINAL) ?? vscode2.window.createTerminal({ name: TERMINAL });
      terminal.show();
...
var TaskSession = class _TaskSession {
  ...
  static async start(context) {
    ...
    const epic = await vscode3.window.showInputBox({
      prompt: "Epic key",
      placeHolder: "PLAT-1234",
      validateInput: (v) => v.trim().length === 0 ? "An epic key is required" : void 0
    });
    if (!epic) return void 0;
    const workflowId = await pick(
      catalog.all().map((w) => ({ label: w.label, id: w.id })),
      "Select task type"
    );
    if (!workflowId) return void 0;
    const featureStory = needsFeatureStory(workflowId) ? await vscode3.window.showInputBox({
      prompt: "Feature story",
      placeHolder: "PLAT-4821"
    }) : "";
    if (featureStory === void 0) return void 0;
    const baseBranch = await vscode3.window.showInputBox({
      prompt: "Base branch to work from",
      placeHolder: "develop",
      validateInput: (v) => v.trim().length === 0 ? "A base branch is required" : void 0
    });
    if (!baseBranch) return void 0;
    const workDir = await vscode3.window.showInputBox({
      prompt: "Work directory \u2014 where repositories are cloned",
      value: resolveCodeRoot(config("codeRoot")),
      validateInput: (v) => isAbsolutePath(v.trim()) ? void 0 : "Enter a full path, such as /Users/you/work"
    });
    if (!workDir) return void 0;
    const services = await vscode3.window.showQuickPick(
    ...
    if (!services || services.length === 0) return void 0;
    ...
    if (Object.keys(errors).length > 0) {
      void vscode3.window.showErrorMessage(Object.values(errors).join(" "));
      return void 0;
    }
    return _TaskSession.startWith(context, selection);
  }
  ...
  static async resume(context, taskId) {
    ...
    if (!await ws.verifySnapshot(state.workflowHash)) {
      ...
      void vscode3.window.showWarningMessage(
        "This task's workflow definition has been modified since the task started. Continuing with the modified version; the change has been recorded in the audit log."
      );
    }
    return _TaskSession.open(context, ws, store, workflow, state);
  }
  static async open(context, ws, store, workflow, state) {
    ...
    const session = new _TaskSession(
      workflow,
      engine,
      registry,
      ctx,
    ...
    if ((actionId === "copy" || actionId === "terminal") && step.stepType === "commandExecution") {
      ...
      try {
        ...
        this.bridge.progress(
          stepId,
          actionId === "copy" ? `Copied the commands for ${label}.` : `Staged the commands for ${label} in the terminal \u2014 press Enter to run them.`
        );
      } catch (err) {
  ...
  async rememberEdit(stepId, values) {
    if (editedPrompt(values) === void 0) return;
  ...
  async afterTransition() {
    ...
    const openFolders = (vscode3.workspace.workspaceFolders ?? []).map((f) => f.uri.fsPath);
    ...
    if (choice === "Open workspace") {
      await vscode3.commands.executeCommand("vscode.openFolder", vscode3.Uri.file(file), false);
    }
  }
  watchOutput(context, taskDir) {
    const watcher = vscode3.workspace.createFileSystemWatcher(
      new vscode3.RelativePattern(vscode3.Uri.file(taskDir), "*.md")
    );
...
async function pick(items, placeHolder) {
  const chosen = await vscode3.window.showQuickPick(items, { placeHolder });
  return chosen?.id;
}
async function openCopilotChatBeside() {
  ...
  for (const command of candidates) {
    try {
      ...
      await vscode3.commands.executeCommand("workbench.action.auxiliaryBar.expand").then(
        () => void 0,
        () => void 0
      );
      return;
...
var SetupView = class {
  ...
  async browse() {
    ...
    const chosen = await vscode4.window.showOpenDialog({
      canSelectFolders: true,
      canSelectFiles: false,
      canSelectMany: false,
      openLabel: "Use as work directory",
      defaultUri: current ? vscode4.Uri.file(current) : void 0
    });
    if (!chosen?.[0]) return;
  ...
  async start() {
    ...
    await vscode4.workspace.getConfiguration("aiDevWorkflow").update("codeRoot", selection.workDir, vscode4.ConfigurationTarget.Global);
    try {
