...
class GitClone {
     ...
     */
    constructor(fallbackWorkDir, exists, sink) {
    ...
    plan(ctx) {
        ...
        return this.selected(ctx).map((service) => {
            ...
            const lines = this.exists(this.pathOf(ctx, service))
                ? [`cd ${(0, node_path_1.join)(workDir, repo)}`, 'git fetch origin']
                : [
                    `mkdir -p ${workDir}`,
                    `cd ${workDir}`,
            ...
            if (base)
                lines.push(`git checkout ${base}`, 'git pull');
            return {
    ...
    async deliver(blockId, mode, ctx) {
        ...
        if (mode === 'copy')
            ...
            await this.sink.toTerminal(text);
        return { label, text };
    }
    validate() {
    ...
    pathOf(ctx, service) {
        return (0, node_path_1.join)(this.workDirOf(ctx), folderFor(service));
    }
...
function folderFor(service) {
    return (0, schema_1.repoNameOf)(service.gitLocation) ?? service.shortCode;
}
