...
exports.versionIn = versionIn;
exports.meetsMinimum = meetsMinimum;
 ...
 */
exports.DEFAULT_TOOLS = schema_1.toolsFileSchema.parse([
    {
        id: 'git',
        label: 'Git',
        command: 'git',
...
async function loadTools(path) {
    ...
    let tools;
    try {
        tools = schema_1.toolsFileSchema.parse(parsed);
    }
    catch (err) {
        throw new Error(`Tool config at ${path} is not valid: ${err instanceof Error ? err.message : String(err)}`);
    }
    validateTools(tools);
    return tools;
}
